...
async function sendGetRequest(currentQuestionNumber){
    ...
    const data = await fetch('https://irene.informatik.htw-dresden.de:8888/api/quizzes/'+currentQuestionNumber, {
    method: 'GET',
    headers: headers,
    });
...
async function sendPostRequest(numberAnswer, currentQuestionNumber){
    ...
    const data = await fetch('https://irene.informatik.htw-dresden.de:8888/api/quizzes/2/solve', {
        method: 'POST',
        headers: headers,
        body: "["+numberAnswer+"]"
    });
